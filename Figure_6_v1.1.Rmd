...
subtitle: "Figure 6: MRT67307 and BX765 enhance plasmid trasfection in VSMC"
author: "Shuai Yuan"
...
dfBlot <- data.frame(
  ...
  mutate(
    ...
    imgPath = c(
      glue("{root_dir}/20230602_HASMC_pCIGX-R4_OE0.4ug_12well_MRT67307_0-20uM/20230602_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230602_HASMC_pCIGX-R4_OE0.4ug_12well_MRT67307_0-20uM/20230602_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230602_HASMC_pCIGX-R4_OE0.4ug_12well_MRT67307_0-20uM/20230602_aTub.tif")),
    x = c(0.07, 0.07, 0.05), 
    y = c(0.32, 0.63, 0.76), 
    width = c(0.9, 0.9, 0.9), 
...
vtSample <- c(0, 0, 20*0.5^c(9:0)) %>%
  ...
  str_replace("\\.$", "")
vtSample <- paste("BX765", vtSample, "uM")
...
pltTbl <- data.frame(
  sample = factor(vtSample, levels = vtSample), 
  "pCIGX-CYB5R4 (ug)" = c("0", rep("0.4", times = 11)), 
  "BX765 (uM)" = c(0, 0, 20*0.5^c(9:0)) %>%
  ...
  geom_text(size = ggplot2::.pt, angle = 45) +
  labs(y = NULL, x = NULL) +
  coord_cartesian(clip = "off") +
  theme_yuan_wb() +
  theme(
...
pltBlotHASMCDoseMRT <- pltBlot + pltMw + plot_layout(widths = c(9, 0.5))

show(pltBlotHASMCDoseMRT)
```
...
file.list <- file.list[grepl("HASMC", file.list)]
...
dfData <- map_dfr(file.list, read.xlsx) %>%
  ...
  group_by(Date, Inhibitor.Name) %>%
  mutate(
    Signal.Norm = Signal / Signal[Target == "a-Tubulin"]
  ) %>%
  group_by(Date, Target) %>%
  mutate(
    Value = Signal.Norm / Signal.Norm[Lane == 2]
  )
  ...
  mutate(
    plt = map(data, function(df){
      p <- ggplot(df, aes(x = Group, y = Value)) +
        ...
        annotation_custom(grob = grid::textGrob(label = glue("{unique(Target)}\n(Fold Change)"), rot = 90,  gp = grid::gpar(fontsize = 8, fontface = "bold"), x = -11, default.units = "mm")) + 
        geom_point(size = 1) + 
        guides(fill = FALSE) +
        coord_cartesian(clip = "off") +
        theme_yuan() +
        theme(
          ...
          plot.margin = margin(1, 0, 1, 11, unit = "mm")
        )
...
pltBarHASMCDoseMRT <- wrap_plots(lstPltBar, nrow = 2)
...
pltTbl <- data.frame(
  ...
  pivot_longer(cols = names(.)[2:3], names_to = "treatment") %>%
  ggplot(aes(x = sample, y = treatment, label = value)) +
  geom_text(size = ggplot2::.pt, angle = 45, hjust = 1) +
...
pltTblHASMCDoseMRT <- (pltTbl + pltSpacer) + plot_layout(widths = c(9, 0.5))

show(pltTblHASMCDoseMRT)
```

## Panel A Assembly

```{r fig.height=3.3, fig.width=3.75}
pltA <- (pltBlotHASMCDoseMRT / pltBarHASMCDoseMRT / pltTblHASMCDoseMRT) + plot_layout(height = c(1, 2.5, 0.8), ncol = 1)

show(pltA)
```

# **Panel B: BX765 on HASMC**
...
dfBlot <- data.frame(
  ...
  mutate(
    target = factor(target, levels = target), 
    imgPath = c(
      glue("{root_dir}/20230602_HASMC_pCIGX-R4_OE0.4ug_12well_BX765_0-20uM/20230602_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230602_HASMC_pCIGX-R4_OE0.4ug_12well_BX765_0-20uM/20230602_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230602_HASMC_pCIGX-R4_OE0.4ug_12well_BX765_0-20uM/20230602_aTub.tif")),
    x = c(0.1, 0.1, 0.125), 
    y = c(0.305, 0.61, 0.75), 
    width = c(0.83, 0.83, 0.835), 
    height = c(0.15, 0.1, 0.15), 
    lane = 12
  )
...
pltBlotHASMCDoseBX <- pltBlot + pltMw + plot_layout(widths = c(9, 0.5))

show(pltBlotHASMCDoseBX)
```
...
pltBarHASMCDoseBX <- wrap_plots(lstPltBar, nrow = 2)
...
pltTblHASMCDoseBX <- (pltTbl + pltSpacer) + plot_layout(widths = c(9, 0.5))

show(pltTblHASMCDoseBX)
```

## Panel B Assembly

```{r fig.height=3.3, fig.width=3.75}
pltB <- (pltBlotHASMCDoseBX / pltBarHASMCDoseBX / pltTblHASMCDoseBX) + plot_layout(height = c(1, 2.5, 0.8), ncol = 1)

show(pltB)
```


# **Panel C: MRT67307 on RASMC**
...
dfBlot <- data.frame(
  ...
  mutate(
    target = factor(target, levels = target), 
    imgPath = c(
      glue("{root_dir}/20230529_RASMC_pCIGX-R4_OE0.4ug_12well_MRT67307_0-20uM/20230529_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230529_RASMC_pCIGX-R4_OE0.4ug_12well_MRT67307_0-20uM/20230529_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230529_RASMC_pCIGX-R4_OE0.4ug_12well_MRT67307_0-20uM/20230529_aTub.tif")),
    x = c(0.02, 0.02, 0.036), 
    y = c(0.37, 0.71, 0.82), 
    width = c(0.92, 0.92, 0.92), 
    height = c(0.15, 0.1, 0.15), 
    lane = 12
  )
...
pltBlotRASMCDoseMRT <- pltBlot + pltMw + plot_layout(widths = c(9, 0.5))

show(pltBlotRASMCDoseMRT)
```
...
file.list <- list.files(root_dir, pattern = "\\.xlsx$", full.names = TRUE, recursive = TRUE)
file.list <- file.list[grepl("0-20uM|dilution", file.list)]
file.list <- file.list[grepl("RASMC", file.list)]
...
pltBarRASMCDoseMRT <- wrap_plots(lstPltBar, nrow = 2)
...
pltTblRASMCDoseMRT <- (pltTbl + pltSpacer) + plot_layout(widths = c(9, 0.5))

show(pltTblRASMCDoseMRT)
```

## Panel C Assembly

```{r fig.height=3.3, fig.width=3.75}
pltC <- (pltBlotRASMCDoseMRT / pltBarRASMCDoseMRT / pltTblRASMCDoseMRT) + plot_layout(height = c(1, 2.5, 0.8), ncol = 1)

show(pltC)
```

# **Panel D: BX765 on RASMC**
...
dfBlot <- data.frame(
  ...
  mutate(
    target = factor(target, levels = target), 
    imgPath = c(
      glue("{root_dir}/20230529_RASMC_pCIGX-R4_OE0.4ug_12well_BX765_0-20uM/20230529_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230529_RASMC_pCIGX-R4_OE0.4ug_12well_BX765_0-20uM/20230529_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230529_RASMC_pCIGX-R4_OE0.4ug_12well_BX765_0-20uM/20230529_aTub.tif")),
    x = c(0.1, 0.1, 0.1), 
    y = c(0.33, 0.67, 0.75), 
    width = c(0.85, 0.85, 0.85), 
    height = c(0.15, 0.1, 0.15), 
    lane = 12
  )
...
pltBlotRASMCDoseBX <- pltBlot + pltMw + plot_layout(widths = c(9, 0.5))

show(pltBlotRASMCDoseBX)
```
...
pltBarRASMCDoseBX <- wrap_plots(lstPltBar, nrow = 2)
...
pltTblRASMCDoseBX <- (pltTbl + pltSpacer) + plot_layout(widths = c(9, 0.5))

show(pltTblRASMCDoseBX)
```

## Panel D Assembly

```{r fig.height=3.3, fig.width=3.75}
pltD <- (pltBlotRASMCDoseBX / pltBarRASMCDoseBX / pltTblRASMCDoseBX) + plot_layout(height = c(1, 2.5, 0.8), ncol = 1)
...
ggsave("Figure_6_v1.1.pdf", width = 7.5, height = 6.6, dpi = 600)
```
