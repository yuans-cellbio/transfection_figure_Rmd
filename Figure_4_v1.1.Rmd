...
```{r fig.height=1.9, fig.width=3.75}
...
dfBlot <- data.frame(
  ...
  mutate(
    ...
    imgPath = c(
      glue("{root_dir}/20230330_HAEC_pCIGX-R4_OE0.2ug_12well_MRT67307_5uM_Interval_3D/20230330_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230330_HAEC_pCIGX-R4_OE0.2ug_12well_MRT67307_5uM_Interval_3D/20230330_CYB5R4_GFP.tif"), 
      glue("{root_dir}/20230330_HAEC_pCIGX-R4_OE0.2ug_12well_MRT67307_5uM_Interval_3D/20230330_aTub.tif")),
    x = c(0.04, 0.04, 0.055), 
    y = c(0.35, 0.62, 0.76), 
    width = c(0.9, 0.9, 0.9), 
...
pltTbl <- data.frame(
  ...
  "MRT67307 (5uM, hrs)" = rep(c("0", "0-24", "24-48", "48-72"), times = 3), 
  check.names = FALSE) %>%
...
pltBlotRev <- (pltBlot + pltMw + plot_layout(widths = c(9, 0.5))) / (pltTbl + pltSpacer + plot_layout(widths = c(9, 0.5))) + plot_layout(heights = c(1, 0.9))
pltBlotRev <- pltSpacer - pltBlotRev + plot_layout(widths = c(0, 1))
...
file.list <- file.list[grepl("3D", file.list, perl = TRUE)]
...
dfData <- map_dfr(file.list, read.xlsx) %>%
  ...
  mutate(
    Value = Signal.Norm / Signal.Norm[Lane == 6]
  )
  ...
  mutate(
    post.hoc = map2(data, model, function(x, y){
      ...
      dfPostHoc <- contrast(emm, "pairwise", adjust = "tukey") %>%
        ...
        filter(p.value < 0.05, Group2 == "48-72")
...
pltBarRev <- pltSpacer+ pltBarRev + plot_layout(widths = c(0, 1))
...
```{r fig.height=4.9, fig.width=3.75}
((pltBlotRev & theme(plot.tag.position = c(0.01, 0.9))) - (pltBarRev & theme(plot.tag.position = c(0, 1)))) +
...
ggsave("Figure_4_v1.1.pdf", width = 3.75, height = 4.6, dpi = 600)
```
